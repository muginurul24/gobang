...
- Cloudflare Tunnel mode is handled by the same production entrypoint. Set `PRODUCTION_HTTP_PORT` and `PRODUCTION_HTTPS_PORT` to `127.0.0.1:*`, then run [`deploy/production/deploy.sh`](deploy/production/deploy.sh). The files under `deploy/cloudflare-tunnel/` now act only as compatibility shims plus the Caddy override.
